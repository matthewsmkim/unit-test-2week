...
public class MockServiceTest {
    ...
    public void shoudDothrowExceptionWhenChampionSetNameYasuo(){
        Champion champion = mock(Champion.class);
    ...
    public void shouldAtMostThreeTimesForChampionGetName() {
        ...
        System.out.println("size :: " + champion.getName());
        System.out.println("size :: " + champion.getName());
        System.out.println("size :: " + champion.getName());
    ...
    @Test
    public void 리산드라검색시미드포지션이탑이맞는지확인(){
    ...
    @Test
    public void fakeChampionReturn() {
    ...
    @Test
    public void 여러개이상호출횟수검증(){
        List<Champion> champions = new ArrayList<>();
        champions.add(new Champion("티모", "탑", 3));
        champions.add(new Champion("우디르", "정글", 3));
    ...
    public void 탐켄치를_호출하면_탐켄치정보를_리턴하고_1번이하로_호출되었는지_검증() {
        //given
        given(mockRepository.findByName("탐켄치")).willReturn(new Champion("탐켄치", "서폿", 5));
        //when
